...
def export_network(network: Network, context: ParameterContext[str]) -> str:
    ...
    preambles = set()
    any_record = [
        1 for lyr in network.layers if hasattr(lyr, "record") and len(lyr.record)
    ]
    if any_record:
    ...
    # Simulation run
    runner = pynn.export_split_run(network, network.runtime, pynn.pynn_runner).value
    ...
    if len(get_records):
        ...
        save_output_text = (
            f'np.savez_compressed("{save_filename}", ' f"**{SAVE_VARIABLE_NAME})"
        )
    else:
        save_output_text = ""
