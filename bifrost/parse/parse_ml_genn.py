...
def to_synapse(layer_dict):
    ...
    if not ("params" in layer_dict):
        raise AttributeError(
            "Layer description dictionary should contain "
    ...
    syn_type = layer_dict["params"]["cell"].pop("synapse_type", SynapseTypes.CURRENT)
    syn_shape = layer_dict["params"]["cell"].pop("synapse_shape", SynapseShapes.DELTA)
...
def to_neuron_layer(
    index: int, network_dictionary: Dict, network: Network
) -> NeuronLayer:
    """
    ...
    reset_variables = layer_dictionary["params"]["cell"].pop("reset_variables", None)
...
def to_connection(
    pre: NeuronLayer, post: NeuronLayer, network_dictionary: Dict, network: Network
) -> Connection:
...
def ml_genn_to_network(
    ...
    runtime = adjust_runtime(config.get("runtime", DEFAULT_DT), input_layer)
