...
class SUPPORTED_CONFIGS(Enum):
    ...
    MAX_NEURONS_PER_COMPUTE_UNIT: str = (
        "max_neurons_per_compute_unit"  # e.g. core, chip
    )
...
def export_configurations(
    network: NetworkBase, configurations: Dict[str, Any]
) -> Statement:
    ...
    if (
        SUPPORTED_CONFIGS.MAX_NEURONS_PER_COMPUTE_UNIT in _cfgs
        and SUPPORTED_CONFIGS.MAX_NEURONS_PER_LAYER_TYPE in _cfgs
    ):
        warnings.warn(
    ...
    for config in _cfgs:
        if config == SUPPORTED_CONFIGS.MAX_NEURONS_PER_COMPUTE_UNIT:
            statement += export_max_neurons_per_core(network, configurations[config])
            ...
            warn(f"Configuration {config} is not supported!" "Passing as is :O")
...
def export_max_neurons_per_core(
    network: NetworkBase, configuration: List[Tuple[str, Union[int, Tuple[int]]]]
) -> Statement:
    """
    ...
    """
    template = (
        f"{SIMULATOR_NAME}.set_number_of_neurons_per_core({SIMULATOR_NAME}.{{}}, {{}})"
    )
    return Statement(
...
def export_max_neurons_per_layer_type(
    network: NetworkBase, configuration: List[Tuple[str, Union[int, Tuple[int]]]]
):
    """
