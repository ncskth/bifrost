...
def export_layer_output(layer: OutputLayer, context: ParameterContext[str]) -> Statement:
    sink = layer.sink
...
def export_ethernet_output(layer: OutputLayer, context: ParameterContext[str]) -> Statement:
    source = layer.source
    ...
    population_labels = (f"[f\"{source.variable('')}{{channel}}\" " 
...
def export_dummy_output(layer: OutputLayer, context: ParameterContext[str]) -> Statement:
