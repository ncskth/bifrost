...
def export_grab_recordings_back(layer: Layer) -> Statement:
    ...
    tab = " " * 4
    txt = (
        f"{{channel: {variable_name}[channel].get_data()"
        f" for channel in {variable_name}}}"
    )
...
def export_save_recordings(network: Network) -> Statement:
    tab = " " * 4
    recordings_list = [
        f"\"{lyr.variable('')}\": {export_grab_recordings_back(lyr)}"
        for lyr in network.layers
        if hasattr(lyr, "record") and len(lyr.record)
    ]
    if len(recordings_list):
        ...
        shapes_list = [
            f"\"{lyr.variable('')}\": {export_layer_shape(lyr)}"
            for lyr in network.layers
            if len(lyr.record)
        ]
