...
def get_params_from_dict(
    source, source_type: str, pdict: dict, ignore_keys=("target",), try_reduce=False
):
    ignore_dict = {k: pdict.pop(k, None) for k in ignore_keys}
    check_type = pdict.pop("check")
    assert source_type == get_param(
        source, check_type
    ), "not the same layer type as initially specified"

    params = {
        tgt_param: get_param(source, access, try_reduce)
...
def extract_cell_params(layer, cell_trans):
    ...
    return get_params_from_dict(layer, cell_type, pdict, ignore_keys, try_reduce=True)
...
def extract_layer_params(layer, layer_trans, cell_trans):
    ...
    if layer_type == "Layer":
        layer_type = get_param(layer, SYNAPSE_TYPE_PARAM)
...
def extract_layer(layer, index):
    layer_params, layer_type = extract_layer_params(
        layer, layer_translations, cell_translations
    )
...
def extract_all(mlgenn_network):
    ...
    return {
        f"layer_{layer_idx:03d}": extract_layer(layer, layer_idx)
